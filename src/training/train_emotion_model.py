...
# ----------------------------------------------------------------------
class Config:
    """Central configuration for dataset paths, training, and model outputs."""
    ...
    MODEL_SAVE_PATH = os.path.join(PROJECT_ROOT, "models", "emotion_custom_cnn.keras")
    HISTORY_SAVE_PATH = os.path.join(PROJECT_ROOT, "models", "emotion_custom_cnn_history.json")
    ...
    def print_startup_info(cls):
        """Print resolved dataset paths before loading begins."""
...
class EmotionDataLoader:
    """Scans AffectNet folders and prepares train, validation, and test generators."""
    ...
    def _validate_dataset_folders(self):
        """Validate required Train/Test folders and expected class folders."""
    ...
    def _scan_image_folder(self, root_dir):
        """Scan class folders and use folder names as labels."""
    ...
    def _create_generators(self):
        ...
        # Stratified split preserves class balance in the train and validation sets.
        ...
        # Training data uses light augmentation to improve generalisation.
        ...
        # Validation and test data are only rescaled for unbiased evaluation.
        val_datagen = ImageDataGenerator(rescale=1.0 / 255)
        test_datagen = ImageDataGenerator(rescale=1.0 / 255)

        # Generators use the same class order as the model output layer.
    ...
    def print_generator_sanity_check(self):
        """Print one training batch summary before model fitting."""
...
class CustomEmotionCNN:
    """Custom CNN baseline trained from scratch for eight emotion classes."""
...
class EmotionTrainer:
    """Handles model fitting, checkpointing, evaluation, and history export."""
    ...
    def train(self, train_gen, val_gen, class_weights):
        """Run training with early stopping and model checkpoint."""
    ...
    def save_model(self):
        """Save the final restored model weights after training."""
        os.makedirs(os.path.dirname(self.config.MODEL_SAVE_PATH), exist_ok=True)
    ...
    def save_history(self):
        """Save training history for later plotting and reporting."""
        if not self.history:
