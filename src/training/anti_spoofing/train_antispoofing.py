...
DATA_DIR   = BASE_DIR / "data" / "LCC_FASD"
TRAIN_DIR  = DATA_DIR / "LCC_FASD_training"
DEV_DIR    = DATA_DIR / "LCC_FASD_development"
TEST_DIR   = DATA_DIR / "LCC_FASD_evaluation"
...
train_gen = train_datagen.flow_from_directory(
    ...
    class_mode="binary",       # real=1, spoof=0
...
print(f"Class indices: {train_gen.class_indices}")   # {'spoof': 0, 'real': 1}
...
x = Dropout(0.4)(x)
output = Dense(1, activation="sigmoid")(x)   # binary: real vs spoof
...
history1 = model.fit(
    ...
    epochs=15, # Phase 1 is a fixed warm-up for the head
...
print("\nClassification Report:")
print(classification_report(y_true, y_label, target_names=["spoof", "real"]))
...
plt.figure()
sns.heatmap(cm, annot=True, fmt="d", xticklabels=["spoof", "real"], yticklabels=["spoof", "real"])
